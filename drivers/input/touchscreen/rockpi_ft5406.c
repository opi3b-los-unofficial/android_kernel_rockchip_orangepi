...
static void fts_report_value(struct rockpi_ft5406_data *ts_data)
{
	...
	for (i = 0; i < event->touch_point && i < MAX_TOUCH_POINTS; i++) {
				...
				|| event->au8_touch_event[i] == FT_TOUCH_CONTACT)
		{
			...
			if(!((1 << event->au8_finger_id[i]) & ts_data->known_ids))
				printk(KERN_ERR"Touch id-%d: x = %d, y = %d\n",
	...
	for(i = 0; released_ids && i < MAX_TOUCH_POINTS; i++) {
		if(released_ids & (1<<i)) {
			printk(KERN_ERR"Release id-%d, known = %x modified = %x\n", i, ts_data->known_ids, modified_ids);
			...
			const struct i2c_device_id *id)
{
	...
	input_abs_set_max(input_dev, ABS_MT_POSITION_X, SCREEN_WIDTH - 1);
	input_abs_set_max(input_dev, ABS_MT_POSITION_Y, SCREEN_HEIGHT - 1);
	input_set_capability(input_dev, EV_ABS, ABS_MT_POSITION_X);
	input_set_capability(input_dev, EV_ABS, ABS_MT_POSITION_Y);

	input_mt_init_slots(input_dev, MAX_TOUCH_POINTS, INPUT_MT_DIRECT | INPUT_MT_DROP_UNUSED);
